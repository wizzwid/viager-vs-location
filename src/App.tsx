...
// Nouveau : champ Select cohérent avec Field
...
function Viager() {
  ...
  const [sexe, setSexe] = useState("Femme"); // Dropdown
          ...
          <div className="space-y-3">
            ...
            {mode === "Viager occupé" && (
              <Field
                ...
                value={loyer}
                onChange={setLoyer}
                help="Utilisé pour calculer la décote DUH"
              />
            )}
...
/****************************************
 * COMPOSANT CREDIT IMMOBILIER (avec tableau d'amortissement repliable + EXPORT CSV)
 ****************************************/
function CreditImmo() {
  // Calcul principal
  const [capital, setCapital] = useState("250000");
  const [taux, setTaux] = useState("3,20"); // taux fixe annuel
  const [assurance, setAssurance] = useState("0,30"); // %/an sur capital emprunté (calculé constant sur le capital initial)
  ...
  // --- Tableau d'amortissement repliable + Export CSV ---
  type Row = {
    mois: number;
    echeance: number;
    interets: number;
  ...
  const buildSchedule = (): Row[] => {
    const rows: Row[] = [];
    let crd = vCap;
    const r = vTaux / 100 / 12;
    const mensu = mensuHorsAss; // hors assurance
    const assur = assMens; // constant
    ...
    for (let m = 1; m <= totalMonths; m++) {
      ...
      // Ajustement dernier mois pour éviter CRD négatif dû aux arrondis
      if (m === totalMonths) {
        principal = crd;
      }
  ...
  const schedule = useMemo(buildSchedule, [vCap, vTaux, vAss, vDur, mensuHorsAss, assMens]);
          ...
          <div className="mt-6">
            <div className="flex flex-col md:flex-row gap-3">
              <button
                ...
                className="px-4 py-2 rounded-xl text-sm font-medium bg-gray-200 text-gray-700 hover:bg-gray-300 transition shadow no-print"
              >
                {showTable ? "Masquer le tableau d’amortissement" : "Afficher le tableau d’amortissement"}
              </button>
              <button
                onClick={exportCSV}
                className="px-4 py-2 rounded-xl text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 transition shadow no-print"
              >
                Exporter en CSV
              </button>
            </div>
