...
/*********************
 * NOUVEAU : CALCULATEUR DE PRÊT / CRÉDIT IMMOBILIER
 *********************/
function CreditImmo() {
  ...
  const [tauxBCE, setTauxBCE] = useState("");
  const [oat10, setOat10] = useState("");
  const [tauxMoyenImmo, setTauxMoyenImmo] = useState("");
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Simulez un crédit immobilier : mensualité, intérêts totaux et assurance. Comparez des taux par stratégie." />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Section title="Paramètres – Crédit">
          ...
          <div className="space-y-3">
            <div className="text-sm font-semibold">Références de taux (à jour manuellement)</div>
            <Field label="Taux BCE (réfi) — actuel" suffix="%/an" value={tauxBCE} onChange={setTauxBCE} />
            <Field label="OAT 10 ans — France" suffix="%/an" value={oat10} onChange={setOat10} />
            <Field label="Taux moyen crédit immo (observé)" suffix="%/an" value={tauxMoyenImmo} onChange={setTauxMoyenImmo} />
          </div>
        </Section>
        ...
        <Section title="Résultats – Crédit">
          ...
          <div className="grid md:grid-cols-2 gap-6 mt-4">
            ...
            <div className="bg-white rounded-2xl shadow p-4 text-sm">
              ...
              {[
                ["Location nue", tLoc, setTLoc],
                ["Viager", tVia, setTVia],
                ["SCPI", tScpi, setTScpi],
                ["Local commercial", tCom, setTCom],
              ].map(([label, v, setter]: any, i: number) => {
                const r = toNum(v);
                const m = mkMensu(r);
                return (
                  <div key={i} className="grid grid-cols-3 gap-2 items-center py-1">
                    <div>{label}</div>
                    <input className="border rounded-lg p-1 text-right" value={v} onChange={(e) => setter(e.target.value)} inputMode="decimal" />
                    <div className="text-right font-medium">{fmt(m)} €/mois</div>
                  </div>
                );
              })}
              <div className="text-xs text-gray-500 mt-2">Ajuste les taux selon tes propositions bancaires/courtiers.</div>
            </div>
          </div>
        </Section>
      </div>
